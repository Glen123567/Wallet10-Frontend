...
// --------------------------- Backend API wrapper ---------------------------
// !! IMPORTANT: Update this to your deployed backend URL or use http://localhost:5000 for local development !!
...
const api = {
  ...
  register: async (userData) => {
    const response = await fetch(`${API_BASE}/auth/register`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(userData) });
    return response.json();
  },
  login: async (credentials) => {
    const response = await fetch(`${API_BASE}/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(credentials) });
    return response.json();
  },
  getUsers: async () => {
    const response = await fetch(`${API_BASE}/users`, api._withAuth());
    return response.json();
  },
  deleteAllUsers: async () => {
    const response = await fetch(`${API_BASE}/users`, api._withAuth({ method: 'DELETE' }));
    return response.json();
  },
  getMessages: async (u1, u2) => {
    const response = await fetch(`${API_BASE}/messages/${u1}/${u2}`, api._withAuth());
    return response.json();
  },
  sendMessage: async (messageData) => {
    const response = await fetch(`${API_BASE}/messages`, api._withAuth({ method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(messageData) }));
    return response.json();
  },
  deleteAllMessages: async () => {
    const response = await fetch(`${API_BASE}/messages`, api._withAuth({ method: 'DELETE' }));
    return response.json();
  }
};
...
const AuthStorage = {
  getAuth: () => {
    try { const stored = localStorage.getItem(LOCAL_STORAGE_KEY); return stored ? JSON.parse(stored) : null; } catch (e) { console.error("Failed to parse auth data", e); return null; }
  },
  setAuth: (auth) => {
    try { localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(auth)); } catch (e) { console.error("Failed to save auth data", e); }
  },
  clearAuth: () => {
...
function ValidatedInput({ name, type = "text", placeholder, value, onChange, validator }) { const [err, setErr] = useState(""); const [touched, setTouched] = useState(false); const isValid = touched && !err && value && validator && !validator(value); const c = `w-full px-4 py-3 rounded-lg cyber-input ${touched && err ? "error" : ""} ${isValid ? "success" : ""}`; const hC = (e) => { onChange(e); if (touched && validator) setErr(validator(e.target.value) || ""); }; const hB = () => { setTouched(true); if (validator) setErr(validator(value) || ""); }; return (<div className="w-full"><input name={name} type={type} placeholder={placeholder} value={value} onChange={hC} onBlur={hB} className={c} />{touched && err && (<motion.span initial={{ o: 0, y: -10 }} animate={{ o: 1, y: 0 }} className="validation-error">{err}</motion.span>)}{isValid && (<motion.span initial={{ o: 0, y: -10 }} animate={{ o: 1, y: 0 }} className="validation-success">✓ Valid</motion.span>)}</div>); }
function Navbar({ onBack, onLogout, walletAddress, balances, loadingBalances }) { return (<motion.div initial={{ y: -100, o: 0 }} animate={{ y: 0, o: 1 }} transition={{ type: "spring", stiffness: 80, damping: 20 }} className="w-full max-w-7xl mb-8"><div className="flex items-center justify-center relative bg-black/20 p-4 rounded-lg border border-[var(--border-color)]">{onBack && (<motion.button {...scaleTap} onClick={onBack} className="absolute left-4 flex items-center gap-2 text-[var(--primary-glow)] hover:text-white px-3 py-2"><ArrowLeft size={20} /> Back</motion.button>)}<motion.h1 className="text-3xl font-bold flex items-center gap-4" style={{color: 'var(--primary-glow)', textShadow: '0 0 10px var(--primary-glow)'}} {...floatingAnimation}><WalletIcon size={32} />Cyber Secure Chat</motion.h1>{onLogout && (<motion.button {...scaleTap} onClick={onLogout} className="absolute right-4 flex items-center gap-2 text-[var(--secondary-glow)] hover:text-white px-3 py-2"><LogOut size={20} /> Logout</motion.button>)}</div>{walletAddress && (<motion.div initial={{ o: 0, scale: 0.9 }} animate={{ o: 1, scale: 1 }} transition={{ delay: 0.3 }} className="mt-4 text-center"><div className="cyber-card p-4"><p className="text-sm text-slate-300 mb-2 break-all"><span className="uppercase tracking-wider">Wallet: </span><span className="font-mono text-[var(--primary-glow)]">{walletAddress}</span></p>{loadingBalances ? (<div className="flex items-center justify-center gap-2"><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-4 h-4 border-2 border-[var(--primary-glow)] border-t-transparent rounded-full" /><span className="text-slate-400">Syncing...</span></div>) : balances ? (<motion.div variants={staggerList} initial="hidden" animate="visible" className="flex justify-center gap-6 flex-wrap">{Object.entries(balances).map(([sym, val]) => (<motion.div key={sym} variants={bounceIn} className="bg-black/30 px-3 py-1 rounded-md border border-[var(--border-color)]"><span className="font-semibold text-[var(--primary-glow)]">{sym}:</span> {val}</motion.div>))}</motion.div>) : (<p className="text-slate-400">No balance data</p>)}</div></motion.div>)}</motion.div>); }
function Card({ title, children, footer }) { return (<motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="cyber-card w-full max-w-xl rounded-2xl p-6 relative overflow-hidden"><div className="relative z-10">{title && (<motion.h2 initial={{ y: -20, o: 0 }} animate={{ y: 0, o: 1 }} transition={{ delay: 0.1 }} className="text-2xl font-bold mb-6 text-center uppercase" style={{ color: 'var(--primary-glow)', textShadow: '0 0 8px var(--primary-glow)'}}>{title}</motion.h2>)}<motion.div initial={{ o: 0 }} animate={{ o: 1 }} transition={{ delay: 0.2 }}>{children}</motion.div>{footer && (<motion.div initial={{ y: 20, o: 0 }} animate={{ y: 0, o: 1 }} transition={{ delay: 0.3 }} className="mt-6">{footer}</motion.div>)}</div></motion.div>); }
...
export default function App() {
  ...
  // FIXED connectMetaMask function with message signing
