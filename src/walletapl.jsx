...
const API_URL = "https://wallet10-backend-1.onrender.com";

// This is the "menu" of functions we can call on any standard ERC20 token contract.
const ERC20_ABI = [
...
export default function App() {
  ...
  const fetchAllBalances = useCallback(async (address) => {
    try {
      ...
      setUsdtBalance(formatUnits(usdtRawBalance, usdtDecimals));
      ...
      setUsdcBalance(formatUnits(usdcRawBalance, usdcDecimals));
    } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'create') {
        ...
        if (!walletName.trim() || !password.trim()) { throw new Error("Fill all fields"); }
  ...
  const handlePasswordReset = async () => {
    const lowerCaseWalletName = walletName.trim().toLowerCase();
    if (!lowerCaseWalletName || !mnemonicInput.trim() || !password.trim()) {
    ...
    return (
      <div className="app-pre-login">
        ...
        <div className="login-box">
          <h1 className="title">🦊 CALAMITY WALLET</h1>
          ...
          <div className="input-group">
            <input
              placeholder="Wallet Name (case-insensitive)"
              value={walletName}
  ...
  return (
    <div className="app-logged-in">
      ...
      <header className="app-header">
        <h1 className="title-small">🦊 CALAMITY WALLET</h1>
        <button 
            ...
            {activeTab === 'contacts' && (
              <Card title="Address Book">
                ...
                <div className="contacts-list">
                  ...
                  {contacts.length > 0 ? (<ul>{contacts.map(contact => (<li key={contact._id}><div className="contact-info"><strong>{contact.contactName}</strong><span>{contact.contactAddress}</span></div><button className="btn-delete" onClick={() => handleDeleteContact(contact._id)}>🗑️</button></li>))}</ul>) : <p>You have no saved contacts.</p>}
                </div>
              </Card>
            )}
